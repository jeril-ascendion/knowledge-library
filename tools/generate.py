...
<script type="module">
  ...
  window.__loadTransformers = async function() {{
    ...
    window.__transformersImportPromise = (async () => {{
      ...
      // T7.5 hotfix (corrected diagnosis):
      // Transformers.js v2.17.2 defaults: allowLocalModels=true with
      // localModelPath='/models/'. The library tries local first
      ...
      // skip the local check and go directly to remoteHost.
