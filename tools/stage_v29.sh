...
<h1>Twenty-six pages, twenty-six motion mechanics — Security group fix (strict build)</h1>
...
# ─── v29 cleanup script (deletes orphan stub directories from user's repo) ───
cat > "$REPO/tools/CLEANUP_v29.sh" <<'CLEANUP_EOF'
#!/usr/bin/env bash
...
CLEANUP_EOF
chmod +x "$REPO/tools/CLEANUP_v29.sh"

tar czf "$OUT/ascendion-engineering-v29.tar.gz" \
    --exclude='.git' --exclude='node_modules' --exclude='*.pyc' \
    tools src content infra .github

# Also expose CLEANUP_v29.sh as a standalone output (user can run it before
# extracting the tarball, without having to extract the tarball first).
cp "$REPO/tools/CLEANUP_v29.sh" "$OUT/CLEANUP_v29.sh"
...
cp "$PREVIEW/twentysix-emblems.html"                       "$OUT/twentysix-emblems-v29.html"
cp "$PREVIEW/patterns-data-preview.html"                  "$OUT/data-patterns-preview-v29.html"
cp "$PREVIEW/knowledge-graph-preview.html"                "$OUT/knowledge-graph-preview-v29.html"
cp "$PREVIEW/system-design-edge-ai-preview.html"          "$OUT/edge-ai-preview-v29.html"
cp "$PREVIEW/system-design-event-driven-preview.html"     "$OUT/event-driven-preview-v29.html"
cp "$PREVIEW/system-design-ha-dr-preview.html"            "$OUT/ha-dr-preview-v29.html"
cp "$PREVIEW/system-design-scalable-preview.html"         "$OUT/scalable-preview-v29.html"
cp "$PREVIEW/technology-ui-ux-cx-preview.html"            "$OUT/ui-ux-cx-preview-v29.html"
cp "$PREVIEW/technology-api-backend-preview.html"         "$OUT/api-backend-preview-v29.html"
cp "$PREVIEW/technology-databases-preview.html"           "$OUT/databases-preview-v29.html"
cp "$PREVIEW/technology-cloud-preview.html"               "$OUT/cloud-preview-v29.html"
cp "$PREVIEW/technology-devops-preview.html"              "$OUT/devops-preview-v29.html"
cp "$PREVIEW/technology-practice-circles-preview.html"    "$OUT/practice-circles-preview-v29.html"
cp "$PREVIEW/technology-engagement-models-preview.html"   "$OUT/engagement-models-preview-v29.html"
cp "$PREVIEW/security-application-security-preview.html"           "$OUT/application-security-preview-v29.html"
cp "$PREVIEW/security-authentication-authorization-preview.html"   "$OUT/authentication-authorization-preview-v29.html"
cp "$PREVIEW/security-cloud-security-preview.html"                 "$OUT/cloud-security-preview-v29.html"
cp "$PREVIEW/security-encryption-preview.html"                     "$OUT/encryption-preview-v29.html"
cp "$PREVIEW/security-vulnerability-management-preview.html"       "$OUT/vulnerability-management-preview-v29.html"
...
tar tzf "$OUT/ascendion-engineering-v29.tar.gz" | head -30
